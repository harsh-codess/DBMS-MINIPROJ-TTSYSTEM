...
import { getFacultyTimetableReport, listPeriods } from "@/lib/queries";
...
export default async function FacultyReportPage() {
  ...
  // Group by faculty
  const byFaculty = new Map<string, typeof entries>();
  for (const entry of entries) {
    if (!byFaculty.has(entry.full_name)) {
      byFaculty.set(entry.full_name, []);
    }
    byFaculty.get(entry.full_name)!.push(entry);
  }

  // Create empty state if no faculty have timetables
  if (byFaculty.size === 0) {
    return (
      <AppShell title="Faculty Timetable">
    ...
    <AppShell title="Faculty Timetable">
      ...
      <div className="mb-6 flex items-center justify-between print:hidden">
        <p className="text-[13px] text-[#6B6B6B]">
          Printable view. Each faculty member will print on a new page.
        </p>
        <PrintButton />
      </div>

      <div className="space-y-12 print:space-y-0 print:block">
        {Array.from(byFaculty.entries()).map(([facultyName, facultyEntries]) => (
          <div key={facultyName} className="print:break-after-page print:pt-4 w-full">
            ...
            <div className="w-full border-2 border-black bg-[#FFF200] text-center py-2 mb-4">
              ...
              <p className="text-[11px] print:text-[10px] font-semibold">
                DUTY WINDOW: {facultyEntries[0]?.duty_window}
              </p>
            </div>
                  ...
                  {weekdays.map((wd, dayIdx) => (
                    <tr key={wd}>
                      ...
                      {fixedSlots.map((slot, i) => {
                        ...
                        const cellEntries = dbPeriod ? facultyEntries.filter(
                                ...
                                {cellEntries.map((ce, idx) => (
                                  <div key={idx} className="leading-tight">
                                    ...
                                    <span className="text-[8.5px] sm:text-[10px] text-black block">
                                      {ce.panel}{ce.lab_batch ? ` (${ce.lab_batch})` : ''} · {ce.room}
                                    </span>
                                  </div>
                                ))}
                              </div>
